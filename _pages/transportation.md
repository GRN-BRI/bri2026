---
title: "GRN-BRI - transportation"
layout: textlay
excerpt: "GRN-BRI -- transportation"
sitemap: false
permalink: /transportation
---


# Transportation Guide
The conference venue is located at **The International Convention Center, Jeonbuk National University** in Jeonju city.

## Arrival at Incheon International Airport (ICN)
#### Option A: Via Jeonju Intercity Bus Terminal (Recommended，Closest to the venue.）
Step 1: Incheon Airport $\rightarrow$ Jeonju Intercity Bus Terminal
 - Mode: Intercity Bus
 - Travel Time: Approx. 3 hours
 - Fare: Approx. KRW 31,000
 - Ticket Purchase: Online or On-site at the Airport <br>
 
Step 2: Jeonju Intercity Bus Terminal $\rightarrow$ Venue
 - Mode: Taxi
 - Travel Time: Approx. 10 minutes (3.6 km)
 - Fare: Approx. KRW 6,000
 <br>
 
#### Option B: Via Jeonju Limousine Bus Terminal(Alternative bus route)
Step 1: Incheon Airport $\rightarrow$ Jeonju Limousine Bus Terminal
 - Mode: Limousine Bus (Daehan Limousine)
 - Travel Time: Approx. 3.5 – 4 hours
 - Fare: Approx. KRW 33,000 – 34,000
 - Ticket Purchase:Online or On-site at the Airport.
Step 2: Jeonju Limousine Bus Terminal $\rightarrow$ Venue
 - Mode: TaxiTravel 
 - Time: Approx. 20 minutes (7 km)
 - Fare: Approx. KRW 10,000
<br>

#### Option C: Via KTX Train (High-Speed Train) (Recommended if you prefer trains or plan to stop by Seoul).

Step 1: Incheon Airport $\rightarrow$ Seoul Station
 - Mode: AREX (Airport Railroad Express)
 - Travel Time: 43 – 51 mins (Express Train) / 60 mins (All-stop Train)
 - Fare: Approx. KRW 9,500 (Express)

Step 2: Seoul Station $\rightarrow$ Jeonju Station
 - Mode: KTX 
 - Travel Time: Approx. 1 hour 40 minutes
 - Fare: Approx. KRW 34,600 (Economy Class)
 - Ticket Purchase:Online or On-site at the station.

Step 3: Jeonju Station $\rightarrow$ Venue
 - Mode: Taxi
 - Travel Time: Approx. 10 – 15 minutes (4 km)
 - Fare: Approx. KRW 6,000

<br>


## Arrival at Gimpo International Airport (GMP)
#### Option A: Via Jeonju Limousine Bus Terminal(Most convenient route from Gimpo)
Step 1: Gimpo Airport $\rightarrow$ Jeonju Limousine Bus Terminal
 - Mode: Limousine Bus
 - Travel Time: Approx. 3.5 hours
 - Fare: Approx. KRW 28,000
 - Ticket Purchase:Online or On-site at the Airport

Step 2: Jeonju Limousine Bus Terminal $\rightarrow$ Venue
 - Mode: Taxi
 - Travel Time: Approx. 20 minutes (7 km)
 - Fare: Approx. KRW 10,000
<br>

#### Option B: Via KTX Train (High-Speed Train) (Seoul Station)
Step 1: Gimpo Airport $\rightarrow$ Seoul Station
 - Mode: AREX (Airport Railroad)
 - Direction: Bound for Seoul Station.
 - Travel Time: Approx. 30 minutes.
 - Fare: Approx. KRW 1,750 (Use T-money card or buy a single journey ticket).
 
Step 2: Seoul Station $\rightarrow$ Jeonju Station
 - Mode: KTX
 - Travel Time: Approx. 1 hour 40 minutes.
 - Fare: Approx. KRW 34,600.
 - Ticket Purchase: Online or On-site at the station.

Step 3: Jeonju Station $\rightarrow$ Venue
 - Mode: TaxiTravel
 - Time: Approx. 10 – 15 minutes.
 - Fare: Approx. KRW 7,000.










